...
export class GameManager {
    ...
    loadLevel(level) {
        ...
        const createBlock = (
            ...
            if (color === this.wallColor) {
                ...
                switch (orientation) {
                    case "row":
                        ...
                        minPoint = new THREE.Vector3(
                            cube.position.x - (BLOCK_SIZE / 2) * cube.scale.x,
                            cube.position.y - (BLOCK_SIZE / 2) * cube.scale.y,
                            cube.position.z -
                                (BLOCK_SIZE / 2) * (cube.scale.z + 0.1)
                        );
                    ...
                    case "collumn":
                        ...
                        minPoint = new THREE.Vector3(
                            cube.position.x -
                                (BLOCK_SIZE / 2) * (cube.scale.x + 0.1),
                            cube.position.y - (BLOCK_SIZE / 2) * cube.scale.y,
                            cube.position.z - (BLOCK_SIZE / 2) * cube.scale.z
                        );
                ...
                let wall = new CollisionBlock(BLOCK_SIZE);
                if (minPoint)
        ...
        const createMergedGroupRow = (groupSize, i, j) => {
            ...
            for (let n = initialCoordinate; n <= j; n++) {
                if (n == mediumCoordinate) {
                    ...
                    dataCopy[n][i].type = "WasMergedBlock";
                }
            }
        ...
        const createMergedGroupCollumn = (groupSize, i, j) => {
            ...
            for (let n = initialCoordinate; n <= j; n++) {
                if (n == mediumCoordinate) {
                    ...
                    dataCopy[i][n].type = "WasMergedBlock";
                }
            }
        };
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            for (let j = 0; j < data[0].length; j++) {
                if (lastIterationWasWall === false) {
                    if (
                        data[i][j].type == "WallBlock" ||
                        data[i][j].type == "WasMergedBlock"
                    ) {
                    ...
                    if (
                        data[i][j].type == "WallBlock" ||
                        dataCopy[i][j].type == "WasMergedBlock"
                    ) {
                        groupSize += 1;
                        ...
                        if (groupSize > 1)
                            createMergedGroupCollumn(groupSize, i, j - 1);
                        lastIterationWasWall = false;
                    }
                }
            }
        ...
        for (let i = 0; i < data[0].length; i++) {
            ...
            for (let j = 0; j < data.length; j++) {
                if (lastIterationWasWall === false) {
                    if (
                        data[j][i].type == "WallBlock" ||
                        data[j][i].type == "WasMergedBlock"
                    ) {
                        lastIterationWasWall = true;
                        groupSize = 1;
                    } else {
                    ...
                    if (
                        ...
                        groupSize += 1;
                        if (j + 1 == data.length)
                            createMergedGroupRow(groupSize, i, j);
                    } else {
                        if (groupSize > 1)
                            createMergedGroupRow(groupSize, i, j - 1);
                        lastIterationWasWall = false;
                    }
                }
            }
    ...
    keyboardUpdate() {
        ...
        // console.log(this.players[0].tank.position);
        // console.log(this.players[0].tank.rotation);

        this.players[0].tank.model.position.copy(
            new THREE.Vector3(48.494277988944226, 9.9, 59.039989489215365)
        );
        this.players[0].tank.rotation.copy(
            new Euler(
                -3.141592653589793,
                -0.787500000000001,
                -3.141592653589793
