...
export function setDefaultMaterial(color = "rgb(255,20,20)", tex = null) {
    ...
    if (!tex) basicMaterial = new THREE.MeshLambertMaterial({ color: color });
    else basicMaterial = new THREE.MeshLambertMaterial({ map: tex });
...
export function initRenderer2d(color = "rgb(0, 0, 0)") {
    //var props = (typeof additionalProperties !== 'undefined' && additionalProperties) ? additionalProperties : {};
...
export function addLargeGroundPlane(scene, useTexture) {
    ...
    if (withTexture) {
        ...
        planeMaterial.map = textureLoader.load(
            "../../assets/textures/general/floorWood.jpg"
        );
