...
export async function loadLights() {
    ...
    for (const index in levelPaths) {
        ...
        const loaded_level = await fetch(path);
        const level_json = await loaded_level.json();
        const level_data = level_json.lighting;
        lighting[index] = level_data;
    }
}
...
export function getLevel(_index) {
    index = _index;
    if (levels[_index]) return levels[_index];
    return levels[0];
}

export function getLights(ind = null) {
    if (ind) return lighting[ind];
    else return lighting[index];
}

export function getTurrets(ind = null) {
    if (ind) return turrets[ind];
    else return turrets[index];
}
